...
-   The **`target_context`** you want Puppilot to reference.
...
-   **Iterative Information Refinement:** Engage in a step-by-step dialogue with Puppilot, building upon previous interactions (captured in `chat_history`) to explore a topic thoroughly, all while keeping an external document (in `target_context`) as a central reference point.
...
-   **`chat_history`** (array of objects, optional): An array detailing the previous turns in your current conversation with Puppilot. Each object in the array represents one prior user input and Puppilot's corresponding output:
...
-   **`output`** (string): Puppilot's generated textual response, informed by your `message`, the `target_context`, and the `chat_history`.
